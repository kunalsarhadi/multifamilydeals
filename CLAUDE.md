...
**Card frame is 16/10 with `object-fit:cover`.** Check the source ratio before committing: elevations run 1.12–1.66. Anything near 1.6 is fine; a near-square one (the 9-Plex Inglewood and the second West Jasper are ~1.13) gets scaled ~1.4x and loses the roof and the entry steps. Fix by compositing the FULL uncropped elevation onto a 1600x1000 canvas over a blurred, cover-scaled copy of itself (see `images/inv/west-jasper-place-2-8plex.png`) — the card stays edge-to-edge like its siblings and nothing is cropped. Do not crop the building to force 16/10, and do not change the card frame (it would break the grid). If localizing is not possible, reference Drive directly — `https://lh3.googleusercontent.com/d/<FILE_ID>=w1600` — and always include the graceful fallback (`onerror` hides the `<img>` + its "Artist's rendering" tag so the card degrades to the placeholder panel rather than showing a broken image). Tell the owner to eyeball it live, and offer to localize to `images/inv/<neighborhood>-<n>plex.png` when they can supply the file. **Never** reuse another property's rendering to fill the gap.
